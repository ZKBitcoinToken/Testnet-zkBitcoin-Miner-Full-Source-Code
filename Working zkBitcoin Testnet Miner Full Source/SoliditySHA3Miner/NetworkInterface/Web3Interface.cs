    ...
    public class Web3Interface : NetworkInterfaceBase
    {
        ...
        public override bool SubmitSolution(string address, byte[] digest, byte[] challenge, HexBigInteger difficulty, byte[] nonce, object sender)
        {
            ...
            {  //this goes down to line 1356
                try
                {
                  ...
                  //  Console.WriteLine("retry count" + retryCount);
                    ...
                    m_ResetIfEpochGoesUp = (int)epochNumber5555;
                    if (currentCounter >=(int)epochNumber5555 && RunThisIfExcessMints)
                    {
            ...
            */
                    ...
                    if (currentCounter < m_MaxZKBTCperMint / 50 && new BigInteger(currentCounter + m_HowManyBlocksAWAYFromAdjustmentToSendMinimumZKBTC) <= (epochNumber5555))
                    {
                    ...
                    if (currentCounter < m_MinZKBTCperMint / 50)
                    {
                        Program.Print(string.Format("STILL SOLVING, Total Good Solves count: " + currentCounter));
                        Program.Print(string.Format("STILL SOLVING, Solves til transaction sending: " + thiszzzzz));
                   ...
                   // Console.WriteLine($"2Updated Counter: {currentCounter}");
                    ...
                    if (filteredLastNonceArray2x.Count < miniz222 && filteredLastNonceArray2x.Count < m_MaxZKBTCperMint / 50 && new BigInteger(currentCounter + m_HowManyBlocksAWAYFromAdjustmentToSendMinimumZKBTC) <= (epochNumber5555))
                    {
                    ...
                    if (filteredLastNonceArray2x.Count < miniz222 && filteredLastNonceArray2x.Count < m_MinZKBTCperMint / 50)
                    {
                        Program.Print(string.Format("STILL SOLVING Total Good Solves count: " + filteredLastNonceArray2x.Count));
                        Program.Print(string.Format("STILL SOLVING Solves til mint: " + (miniz222 - filteredLastNonceArray2x.Count)));
                    ...
                    var miniz = Math.Min(currentCounter, Math.Min(lastNonceArray2x.Count, _BLOCKS_PER_READJUSTMENT_ + (int)epochNumber5555));
                    ...
                    do
                    {
                        ...
                        try
                        {
                           ...
                           // Program.Print("HEX 2STUFF: " + combinedHexString);
                            ...
                            while ((transactionID == null && stopwatch.ElapsedMilliseconds < 15500) && OnlyRunPayMasterOnce && m_ETHwithMints)
                            {
        ...
        public Web3Interface(string web3ApiPath, string contractAddress, string minerAddress, string privateKey,
                             ...
                             ulong gasLimit, string gasApiURL, string gasApiPath, string gasApiURL2, string gasApiPath2, string gasApiPath3, bool ETHwithMints, string[] AddressesOfERC20ToMint, float gasApiMultiplier, float MaxZKBTCperMint, float MinZKBTCperMint, float HowManyBlocksAWAYFromAdjustmentToSendMinimumZKBTC, float gasApiOffset, float gasApiMax)
            : base(updateInterval, hashratePrintInterval)
        {
            ...
            else
            {
                ...
                mintABI = contractABI.Functions.FirstOrDefault(f => f.Name == "multiMint_SameAddress_EZ");
                if (mintABI != null) m_mintMethod = m_contract.GetFunction(mintABI.Name);
                mintABIwithETH = contractABI.Functions.FirstOrDefault(f => f.Name == "multiMint_SameAddress_EZ");
